...
namespace FrOnDaL_AIO.Common.Utilities.SpellBlocking
{
    internal class EvadeTargetManager
    {
        ...
        {
            Menu = new Menu("EvadeTargetMenu", "Blocking directivity skills");
            {
                Menu.Add(new MenuBool("Brian.EvadeTargetMenu.EvadeTargetW", "Enable"));
                Menu.Add(new MenuBool("Brian.EvadeTargetMenu.CC", "Teammates charged with automatic shield"));
            }
            ...
            WhiteList = new Menu("whitelist", "套盾白名单");
            {
                ...
                {
                    WhiteList.Add(new MenuBool(target.CharacterName.ToLower(), "Enabled: " + target.CharacterName));
                }
            }
            Menu.Add(WhiteList);

            InitSpells();
            AttackMenu = new Menu("Brian.EvadeTargetMenu.DodgeAttackMenu", "Set of shields");
            {
                AttackMenu.Add(new MenuBool("Brian.EvadeTargetMenu.BAttack", "General attack"));
                AttackMenu.Add(new MenuSlider("Brian.EvadeTargetMenu.BAttackHpU", "^- If blood volume <=", 80, 1, 100));
                AttackMenu.Add(new MenuBool("Brian.EvadeTargetMenu.CAttack", "Crit"));
                AttackMenu.Add(new MenuSlider("Brian.EvadeTargetMenu.CAttackHpU", "^- If blood volume <=", 80, 1, 100));
                AttackMenu.Add(new MenuBool("Brian.EvadeTargetMenu.Turret", "Defense tower attack"));
                AttackMenu.Add(new MenuBool("Brian.EvadeTargetMenu.Minion", "Small attack"));
                AttackMenu.Add(new MenuSlider("Brian.EvadeTargetMenu.HP", "^- If blood volume <=", 20, 1, 100));
            }
            Menu.Add(AttackMenu);
                ...
                if (aaa == ally)
                {
                        ...
                        ObjectManager.Player.CharacterName == "Karma")
                    {
                        if (Menu["whitelist"][
                            ...
                            aaa.Distance(ObjectManager.Player) < spellMisc.E.Range)
                        {
                            spellMisc.E.CastOnUnit(aaa);
                        }
                    }
                    if (ObjectManager.Player.CharacterName == "Lux" || ObjectManager.Player.CharacterName == "Sona" ||
                        ObjectManager.Player.CharacterName == "Taric")

                    {
                        if (Menu["whitelist"][
                                    ally.CharacterName.ToLower()] &&
                            aaa.Distance(ObjectManager.Player) < spellMisc.W.Range)
                        {
                            spellMisc.W.CastOnUnit(aaa);
                        }
                    }
                }
